...
<template>
    ...
    >
        <div class="grid grid-cols-5 border-y py-[8px] border-gray-200">
            <span class="col-span-3 pl-[6px]">{{ $t('wallet.receiverAccount') }}</span>
            <span class="col-span-2 text-center">{{ $t('wallet.transationTime') }}</span>
        </div>
        ...
        <n-scrollbar>
            ...
            <div
                class="text-center py-[10px] text-gray-400"
                v-if="wallet.recentTransations.length === 0"
            >
                {{ $t('public.noData') }}
            </div>
        </n-scrollbar>
